...
Alamo Drafthouse API + BAM scraper
        ↓
Merge
        ↓
TMS/Gracenote enrichment
        ↓
OMDb enrichment
        ↓
...
class MoviePipeline:
    ...
    def fetch_movies(self):
        ...
        print("=" * 60)
        print("Fetching Alamo Drafthouse Brooklyn showtimes")
        print("=" * 60)

        try:
            alamo = AlamoFetcher()
            self.movies = alamo.fetch_movies()
        except Exception as ex:
            # Alamo's API is out of our control. Don't let it take down
            # the whole run -- BAM's data is still worth writing.
            print(f"Alamo fetch failed, continuing without it: {ex}")
            self.movies = []

        print()

        print(f"Fetched {len(self.movies)} unique movies from Alamo Drafthouse.")

        # Fetch BAM movies and merge them in
        print()
        print("=" * 60)
        print("Fetching BAM Rose Cinemas showtimes (scraped)")
        print("=" * 60)

        try:
            bam = BAMFetcher()
            bam_movies = bam.fetch_movies()
        except Exception as ex:
            # BAM's site is out of our control. Don't let it take down
            # the whole run -- Alamo's data is still worth writing.
            print(f"BAM fetch failed, continuing without it: {ex}")
            bam_movies = []

        print(f"Fetched {len(bam_movies)} movies from BAM.")

        # Merge BAM movies into Alamo movies using a smarter
        ...
        print(f"Merged BAM movies: {added} new movies added.")

    # ---------------------------------------------------------

    def enrich_with_tms(self):

        print()

        print("=" * 60)
        print("Enriching with TMS/Gracenote")
        print("=" * 60)

        try:
            tms = TMSFetcher()
            self.movies = tms.enrich(self.movies)
        except Exception as ex:
            # Missing TMS_API_KEY or a TMS outage shouldn't take down the
            # whole run -- OMDb enrichment still fills in what it can.
            print(f"TMS enrichment failed, continuing without it: {ex}")

        print()

        print("Finished TMS enrichment.")
    ...
    def build_changelog_entry(self):
        ...
        entry = {
            "date": datetime.now(ZoneInfo("America/New_York")).strftime("%Y-%m-%d %I:%M %p %Z"),
